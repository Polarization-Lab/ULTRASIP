...
#Work Computer
    ...
    outpath = "C:/Users/ULTRASIP_1/Documents/ULTRASIP/AirMSPI_FIREXAQ/Retrievals/Apr1823/Scat_FIREXTest"
...
#Define i and k for all wavelengths and with zenith angle = 90
        ip = np.array([np.cos(np.radians(saz)), -np.sin(np.radians(saz))]); #illumination vec,flip sign of sza
        ip = ip/np.linalg.norm(ip)

        k_355 = np.array([np.cos(np.radians(vaz_355)), -np.sin(np.radians(vaz_355))])
        k_355 = k_355/np.linalg.norm(k_355);
        
        k_380 = np.array([np.cos(np.radians(vaz_380)), -np.sin(np.radians(vaz_380))]);
        k_380 = k_380/np.linalg.norm(k_380);
        
        k_445 = np.array([np.cos(np.radians(vaz_445)), -np.sin(np.radians(vaz_445))]);
        k_445 = k_445/np.linalg.norm(k_445);
        
        k_470 = np.array([np.cos(np.radians(vaz_470)), -np.sin(np.radians(vaz_470))]);
        k_470 = k_470/np.linalg.norm(k_470);
        
        k_555 = np.array([np.cos(np.radians(vaz_555)), -np.sin(np.radians(vaz_555))]);
        k_555 = k_555/np.linalg.norm(k_555);
        
        k_660 = np.array([np.cos(np.radians(vaz_660)), -np.sin(np.radians(vaz_660))]);
        k_660 = k_660/np.linalg.norm(k_660);
