...
# Change to the figure directory
    ...
    ax[1,0].fill_between(scat[:,3], (q_obs[:,0]-qs[:,0]),(q_obs[:,0]-qs[:,0]), color="blue",alpha=0.4)


    ax[1,1].plot(scat[:,5],q_obs[:,1],linestyle='dashed',color="red",label="660nm")
    ax[1,1].fill_between(scat[:,5], (q_obs[:,1]-qs[:,1]),(q_obs[:,1]-qs[:,1]),color="red",alpha=0.4)


    ax[1,2].plot(scat[:,6],q_obs[:,2],linestyle='dashed',color="orange",label="865nm")
    ax[1,2].fill_between(scat[:,6], (q_obs[:,2]-qs[:,2]),(q_obs[:,2]-qs[:,2]),color="orange",alpha=0.4)
    ...
    ax[2,0].fill_between(np.sort(scat[:,3]), (u_obs[:,0]-us[:,0]),(u_obs[:,0]-us[:,0]),color="blue",alpha=0.4)

    ax[2,1].plot(scat[:,5],u_obs[:,1],linestyle='dashed',color="red",label="660nm")
    ax[2,1].fill_between(scat[:,5], (u_obs[:,1]-us[:,1]),(u_obs[:,1]-us[:,1]),color="red",alpha=0.4)

    ax[2,2].plot(scat[:,6],u_obs[:,2],linestyle='dashed',color="orange",label="865nm")
    ax[2,2].fill_between(scat[:,6], u_obs[:,2]-np.std(u_obs[:,2]), u_obs[:,2]+np.std(u_obs[:,2]),color="orange",alpha=0.4)
